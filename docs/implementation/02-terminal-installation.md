...
Kitty will be installed using the same package source already approved for the Hyprland ecosystem.

The installation should avoid introducing unrelated desktop components.

Once installed, the default Hyprland keybinding (`SUPER + Q`) is expected to launch Kitty without requiring configuration changes.

No modifications to the autogenerated Hyprland configuration are planned during this implementation.
...
- The default shortcut (`SUPER + Q`) opens Kitty.
...
- The default Hyprland keybinding (`SUPER + Q`) launched Kitty without modification.
...
The implementation achieved its intended objective without requiring any manual configuration changes.
